...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kadın <@Salvo/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
    ...
    {
    message.react(red)
    message.lineReply(`Kayıt ettiğin üyenin yaşı 15'ten küçük olamaz.`).then(x => x.delete({timeout: 5000})); 
    return }
    ...
    Kişinin önceki isimlerine \`.isimler @Salvo/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)
